...
unneeded data from the dataframe, and also ensures that there is no grouping.
...
Missing columns can be inserted with `imap`
...
`enum` which accepts a list of values, which the relevel the factor. If
`.drop = TRUE` is specified then values which don't match the levels will be
cast to `NA` rather than causing failure.
