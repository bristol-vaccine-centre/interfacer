...
an roxygen tag for automating documentation of dataframe inputs. This is not
...
dataframe is validated to ensure it is of the right shape. We can use 
`@iparam <param> <description>` in the Roxygen skeleton to document the dataframe
constraints.
...
Demonstrating the behaviour of the `@iparam` roxygen tag is hard in a vignette
...
example `id` (expected to be an int) is provided as a `character` and `extra` 
...
The type of a dataframe column can be defined as a basic datatype, however more
...
`?interfacer::type.default` and `?interfacer::type.in_range`.
...
missing columns, datatype checks and enforcing grouping structure. Automation
