...
an `iface` spec can permissive or dogmatic about the grouping of the input. If
the .groups option in an iface spec is NULL (e.g. `iface(..., .groups=NULL)`)
then any grouping is allowed. If it is FALSE then no grouping is allowd. The
...
`iface` spec. So in this example `cut` is required by the `i_diamonds` contract
...
de provided first. We can leverage this to create a recursive functional pattern of 
