...
If we pass non-conformant data `ivalidate` throws an informative error about what is wrond with the data. In this case the `test` column is missing:
...
We can recover from this error by renaming the columns before passing `bad_example_data` to `example_fn()`. Alternatively we can use the dynamic dots to pass a mapping to `ivalidate` which is applied before the
validation. This parameter is called `.imap`, and its contents is a `imapper()`  that holds an set of expressions used for a `dplyr::mutate` call).
...
To be useful interface specifications can be composed and extended. In this case a subtype of the `iface` specification can be created:
...
and then this extended `iface` used to constrain the input of another function by specifying it as the default value. We can also constrain the output of the function to be conformant to another specification usng `ireturn`. Examples of documenting the input parameter and the output parameter are provided here:
...
unneeded data from the dataframe, removing any extra columns, and also ensures that the input is not grouped in any way. Grouping
...
This is rejected because the grouping is incorrect. An inforative error message is provided:
...
The roxygen block of documentation for this second interface is determined by the `#' @param` block and the function `idocument` which takes a function name and optionally a parameter name and writes an informative block about the dataframe
...
`interfacer` does not implement a rigid type system, but rather a permissve one. 
...
Completely incorrect data types on the other hans are picked up and rejected. In
...
of a provided factor. The order of the levels will be taken from the `iface` spec
and relveleving of inputs is taken to ensure the factor levels match the spec. If
...
Dispatching to one of a number of functions based on the nature of a dataframe input
is enabled by `idispatch(...)`. This emulates the behaviour of `S3` classes but for
dataframes. An example is the following:
...
disp_example.extn = function(x = i_test_extn, ...) {
  # idispatch has already validated x so we don;t need to here
  message("extended data")
  return(colnames(x))
}

disp_example.no_extn = function(x = i_test, ...) {
  # in this toy example we do the same thing in both possilbilities.
...
In this example we note that the order of the rules is important with the more detailed
being matched first. We can leverage this to create a recursive functional pattern of 
dataframe extension that allows multiple inputs to converge on a single output, this
also demonstrates the use of `itest()` which simply checks an input is of a set type:
...
Processing an input of type 2, results in a different path through the data pipeline, but 
the same outcome:
...
`group_unique` or `complete` which enforce uniqueness and completeness.
